...
class Unet2D(nn.Module):
    ...
    def forward(self, x, weights=None):
        if weights == None:
            ...
            logit = conv2d(y1, self.seg1.weight, self.seg1.bias, kernel_size=None, stride=1, padding=0) #+ upsample(y2)
            ...
            logit = conv2d(y1, weights['seg1.weight'], weights['seg1.bias'], kernel_size=None, stride=1, padding=0) 
        
        return logit, F.softmax(logit, dim=1), y1, torch.flatten(F.adaptive_avg_pool2d(x5, 3), start_dim=1)     
